...
const AdvertismentSlider = () => {
  ...
  useEffect(() => {
    axios
      .get(
        "https://employeemanagementapi-ctecekcdhpcedghe.canadacentral-01.azurewebsites.net/api/Courses"
      )
      .then((response) => {
  ...
  return (
    <div className="py-8 bg-gradient-to-r from-[#3954a9] via-[#314277] to-[#7ba6df] flex flex-col items-center">
      ...
      <motion.div variants={textVariant3()} initial="hidden" whileInView="show">
        ...
        <p className="text-lg text-gray-200 text-center mb-6">
          Explore the latest courses and advertisements to help you advance your
          career and skills.
        </p>
      </motion.div>
      ...
      {/* Slider */}
      <div
        className={`w-full max-w-4xl flex justify-center items-center relative ${
          courses.length < 3 ? "justify-center" : ""
        }`}
      >
        {courses.length > 3 && (
          <div
        ...
        <div className="flex space-x-6">
          {displayedCourses.map((course, index) => (
            ...
            >
              <h2 className="text-xl font-semibold text-[#32549c] mb-2">
                {course.courseTitle}
              </h2>
              {/* <p className="text-gray-300 mb-4">{course.description}</p> */}
